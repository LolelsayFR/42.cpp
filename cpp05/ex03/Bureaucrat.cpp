...
/*   Updated: 2025/09/28 16:50:23 by lolelsay         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void Bureaucrat::signForm(AForm& form) {
		std::cout << *this << " aaaaaaaaaaaaaaaaaaaasign " << form.getName() << std::endl;
	try {
		if (this->_grade < 1)
			throw (Bureaucrat::GradeTooHighException());
		if (this->_grade > 150)
...
void Bureaucrat::executeForm(AForm const& form) {
	try {
		...
		std::cout << "Execute form : " << e.what() << std::endl;
		return ;
	}
	try {
		form.execute(*this);
		...
		std::cout << *this << " couldn't executed " << form.getName() << " because " << e.what() << std::endl;
		return ;
	}
